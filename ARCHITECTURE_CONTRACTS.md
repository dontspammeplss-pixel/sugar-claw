...
The resolved ledger is `docs/contracts/open-decisions.md`; the approval record is `records/approvals/gate-1-baseline-rev1.md`; the mandated versioned contracts are in `records/contracts/` (attachment primitive, collision matrix, fixed-step policy, performance thresholds). Contract-only follow-up packet N36–N40 is recorded in `records/task-packets/N36-N40-descent-grip-collision-contracts.md`; it does not authorize implementation until separately accepted.
