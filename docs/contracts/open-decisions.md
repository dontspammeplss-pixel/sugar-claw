...
## Revision 4 — N36–N40 descent, grip validity, collision observability, and collider derivation (2026-08-02)

**Status:** Contract-only; implementation not started.
**Scope:** Eli reported that the claw does not reach the machine base, can grab beside an object, has unclear collision behavior, and lacks a defined policy for assigning collider parameters to meshes.
**Approver:** Developer Brain (technical contract decomposition); product feel and any change to the approved capture definition remain promotion gates. N37's multi-region capture rule is a recommended provisional profile, not an amendment to A-24 until the packet is accepted and a versioned decision is recorded.

The contract packet is `records/task-packets/N36-N40-descent-grip-collision-contracts.md`.

| Node | Contract outcome                                                                                                                                                                                                                                                       |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| N36  | Define an explicit physical base plane and clearance envelope; descent reaches the lowest safe interaction depth or stops only for an identified physical barrier/contact, never an arbitrary animation endpoint.                                                      |
| N37  | Require profile-driven valid capture: containment plus sensor evidence plus the configured multi-region solver-contact pattern over a fixed-step stability window; side grabs, sensor-only contact, partial contact, and wrong-body contact reject.                    |
| N38  | Inventory and trace every claw/object/floor/wall/sensor collider with identity, shape, group/filter, sensor/solver mode, contact state, and barrier response; visual overlap is never collision proof.                                                                 |
| N39  | Permit mesh geometry to generate versioned conservative collider candidates for simple valid meshes, but require authored profiles or explicit rejection for ambiguous/concave/hollow/articulated/invalid geometry; automatic derivation never invents grip semantics. |
| N40  | Integrate descent, valid-grip, collision, derivation, reset, repeatability, regression, typecheck, lint, test, and build evidence before promotion.                                                                                                                    |

**Protected decisions:** the fixed-step policy, collision matrix, physics authority, explicit carry constraint, and no-silent-dependency rule remain unchanged. Any implementation that needs to alter those decisions opens a separate versioned contract revision.
